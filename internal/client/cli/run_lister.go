...
func listAggregated(data payments.ReadOnlyBTree[payments.Payment], from, to *string) {
	...
	for _, row := range stats {
		fmt.Printf("PERIOD: %s (%s - %s), PAYMENTS: %s, AVG: %s, MIN: %s, MAX: %s, DAYS: %s, AVG: %s, MIN: %s, MAX: %s, TOTAL: %s\n", row[0], row[1], row[2], row[4], row[5], row[6], row[7], row[9], row[10], row[11], row[12], row[14])
	}
}
